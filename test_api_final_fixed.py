#!/usr/bin/env python3
"""
Финальное тестирование API функций (исправленная версия)
"""
...
def test_cache_system():
    ...
    try:
        ...
        # Тест LRU eviction
        print("🔄 Тест LRU eviction")
        for i in range(105):  # Превышаем max_size=100
            cache.set(f"key_{i}", f"value_{i}")

        print(f"   Размер кэша после добавления 105 элементов: {len(cache.cache)}")
        print(f"   Первый элемент: {next(iter(cache.cache.keys()))}")
        print()

        # Тест статистики
        print("📊 Статистика кэша")
        stats = cache.get_stats()
        print(f"   Всего элементов: {stats.get('size', 'N/A')}")
        print(f"   Hits: {stats.get('hits', 0)}")
        print(f"   Misses: {stats.get('misses', 0)}")
        print(f"   Hit rate: {stats.get('hit_rate', 0):.2%}")
        print(f"   Использование памяти: {stats.get('memory_usage_mb', 0):.2f} MB")
        print()
